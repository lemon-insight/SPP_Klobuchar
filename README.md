...
### 5. analyze_klobuchar_effect.py

**功能**：Klobuchar 电离层模型改正效果分析脚本，用于对比带和不带电离层改正的定位结果，从多个维度统计分析模型的改善效果。

**主要功能**：
- 解析批量定位输出的结果文件
- 按测站、半球、纬度带、时段、年积日等维度进行统计
- 计算 RMS 改善量、3D 位置改善量及改善率
- 生成 13 幅分析图表
- 输出 8 个汇总 CSV 表格
...
汇总 CSV 表格（output/目录）：
- `file_level_stats.csv`：每个文件的统计数据
- `paired_comparison.csv`：同站同年积日同时段的配对比较数据
- `latitude_band_summary.csv`：按纬度带汇总的统计数据
- `hemisphere_summary.csv`：按半球汇总的统计数据
- `hemi_lat_summary.csv`：按半球和纬度带组合汇总的统计数据
- `time_group_summary.csv`：按时段汇总的统计数据
- `station_summary.csv`：按测站汇总的统计数据
- `doy_time_summary.csv`：按年积日和时段汇总的统计数据

分析图表（output/figures/目录）：
- 13 幅分析图，涵盖纬度、半球、时段、测站等维度的 RMS 和 3D 位置改善效果

**使用方法**：
```bash
cd code/
python analyze_klobuchar_effect.py
```
